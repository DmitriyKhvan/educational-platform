...
const LessonsCalendarHeader: React.FC<LessonsCalendarHeaderProps> = ({ calendarRef }) => {
  const [open, setOpen] = useState(false);
  const { getColorByCourseId, coursesList, colorsReady } = useCourseColors();

  const { goNext, goPrev, goToday, setView, date, view, viewDictionary } = useCalendarControls({
    calendarRef,
