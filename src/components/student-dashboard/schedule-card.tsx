...
const ScheduleCard = ({
  ...
  const joinLesson = () => {
    ...
    if (
      ...
      window.open(user?.role === Roles.MENTOR ? playground.startUrl : playground.joinUrl, '_blank');
    } else {
    ...
    >
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            {user?.role === Roles.MENTOR && (
              <p
                className={`text-sm ${
                  index === 0 ? 'text-color-light-purple' : 'text-color-light-grey'
                }`}
              >
                {student?.user?.email}
              </p>
      ...
      {LessonsStatusType[data?.status?.toUpperCase() as keyof typeof LessonsStatusType] ? (
        <div className="flex flex-wrap items-center gap-2 xl:gap-3">
          ...
          <a
            onClick={data.status === LessonsStatusType.APPROVED ? joinLesson : undefined}
            target="_blank"
            rel="noreferrer"
