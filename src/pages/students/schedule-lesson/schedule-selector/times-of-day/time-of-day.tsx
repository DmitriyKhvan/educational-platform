...
export const TimeOfDay = memo(function TimeOfDay({ timeOfDay, idx }:{timeOfDay: string , idx: number}) {
  const {
  ...
  const selectTimeOfDay = () => {
    ...
    if (timeOfDay === 'Morning') {
      if (differenceInHours(morningInterval.start, todayUserTimezone) <= hourPrior) {
        setTimeOfDayInterval({
    ...
    if (timeOfDay === 'Afternoon') {
      if (differenceInHours(afternoonInterval.start, todayUserTimezone) <= hourPrior) {
        setTimeOfDayInterval({
          start: addHours(todayUserTimezone, hourPrior),
    ...
    if (timeOfDay === 'Evening') {
      if (differenceInHours(eveningInterval.start, todayUserTimezone) <= hourPrior) {
        setTimeOfDayInterval({
          start: addHours(todayUserTimezone, hourPrior),
