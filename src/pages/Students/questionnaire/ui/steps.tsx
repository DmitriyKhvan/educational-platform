...
export const Steps: React.FC<StepProps> = ({ setCache, questionnaire }) => {
  ...
  const [createMatchingProfileForStudent] = useMutation(CREATE_MATCHING_PROFILE_FOR_STUDENT);
  ...
  useEffect(() => {
    const subscription = watch((value, { name }) => {
      if (name === 'availabilities.days' || name === 'availabilities.time') {
        ...
        setCache((prev) => {
          localStorage.setItem('questionnaire', JSON.stringify({ ...prev, [name]: value[name] }));
