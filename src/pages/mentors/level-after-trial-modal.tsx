...
const LevelAfterTrialModal = () => {
  ...
  const [getLevels, { data: levelsData }] = useLazyQuery(LANGUAGE_LEVELS_WITH_PAGINATION, {
    variables: { limit: 999 },
  });

  const [changeStudentLevel, { loading: mutationLoading }] = useMutation(CHANGE_STUDENT_LEVEL);
  ...
  return (
    <AdaptiveDialog open={isOpenModal} hideCloseBtn={true}>
      <form onSubmit={handleSubmit(onSubmit)}>
        <div className="mb-7">
          <h2 className="text-xl font-bold text-color-dark-violet mb-4">Trial student level</h2>
          <p className="text-color-dark-violet">Determine the final level of your trial student</p>
        </div>

        <div className="flex gap-3 bg-color-bg-grey2 rounded-lg px-4 py-3 mb-7">
          <Avatar fallback="duck" className="bg-color-purple rounded-full w-10 h-10" />
          <div>
            <p className="text-color-dark-violet text-sm font-medium">
              {lesson?.student?.firstName} {lesson?.student?.lastName}
            </p>
            <p className="text-color-purple text-sm">{lesson?.student?.languageLevel?.title}</p>
          </div>
        </div>

        <div className="mb-6">
          <p className="mb-3 text-[15px] font-semibold text-color-dark-violet">Final level</p>
          <MyDropdownMenu
            open={isOpenDropdown}
