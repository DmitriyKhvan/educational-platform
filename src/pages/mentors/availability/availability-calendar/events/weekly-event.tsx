    ...
    >
      <div
        className={cn(
          ...
          data.type === EventType.REGULAR && 'bg-color-purple bg-opacity-15 backdrop-blur-[2px]',
          data.type === EventType.TRIAL && 'bg-[#FF9335] bg-opacity-15 backdrop-blur-[2px]',
          data.type === EventType.LESSON && 'border-l-4 bg-opacity-15 rounded-[4px] mt-[2px] px-2',
      ...
      >
        <p className="w-full overflow-hidden truncate">{eventInfo.event.title}</p>
      </div>
    </div>
  );
};
