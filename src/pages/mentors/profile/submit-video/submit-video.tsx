...
const SubmitVideo = () => {
  ...
  const [updateMentor, { loading, error }] = useMutation(MUTATION_UPDATE_MENTOR);
  ...
  return (
    <div className="p-[50px]">
      <h2 className="text-[40px] leading-[48px] trackign-[-1px]">{t('edit_profile')}</h2>

      <form onSubmit={handleSubmit(handleEditVideo)} className="sm:w-1/2 xl:w-[380px]">
        <div>
          ...
          <InputWithError errorsField={errors?.videoUrl}>
            <InputField
              className="w-full"
              placeholder={typeVideo === 'yt' ? 'youtube.com/video' : 'vimeo.com/video'}
              {...register('videoUrl', {
        ...
        <div className="flex gap-[10px] mt-[35px]">
          ...
          <Button className="w-full" theme="outline" type="submit" disabled={!isValid}>
            {loading ? <ClipLoader loading={loading} size={20} color="white" /> : 'Submit My Video'}
          </Button>
        </div>
      </form>
    </div>
